...
int wmain() {
  ...
  currentPath +=
      L"EffekseerForYMM4_UnicodeIo_" + std::to_wstring(GetCurrentProcessId());

  std::vector<std::wstring> createdDirectories;
  auto cleanup = [&]() {
    for (auto it = createdDirectories.rbegin(); it != createdDirectories.rend();
         ++it) {
  ...
  const auto combinedFilePath =
      EffekseerForYMM4::WindowsString::CombineUtf16Path(
          reinterpret_cast<const char16_t *>(currentPath.c_str()),
          u"実ファイル_\U0001F60A.bin");
  if (combinedFilePath.size() <= 512 ||
      combinedFilePath.find(u'\\') != std::u16string::npos) {
    std::cerr << "Long UTF-16 path combination did not preserve the full "
                 "normalized path."
              << std::endl;
    cleanup();
    return 1;
  ...
  const auto parentPath = EffekseerForYMM4::WindowsString::GetParentUtf16Path(
      combinedFilePath.c_str());
  const std::u16string expectedParent(
      reinterpret_cast<const char16_t *>(currentPath.c_str()));
  auto normalizedExpectedParent = expectedParent;
  std::replace(normalizedExpectedParent.begin(), normalizedExpectedParent.end(),
               u'\\', u'/');
  if (parentPath != normalizedExpectedParent) {
    std::cerr << "Long UTF-16 parent path extraction truncated the path."
              << std::endl;
    cleanup();
    return 1;
  }
  ...
  const auto fileHandle =
      CreateFileW(extendedFilePath.c_str(), GENERIC_WRITE, FILE_SHARE_READ,
  ...
  constexpr std::array<char, 23> expected = {
      'e', 'f', 'f', 'e', 'k', 's', 'e', 'e', 'r', '-', 'l', 'o',
      'n', 'g', '-', 'p', 'a', 't', 'h', '-', 'i', 'o', '!'};
  DWORD bytesWritten = 0;
  const auto writeSucceeded =
      WriteFile(fileHandle, expected.data(),
                static_cast<DWORD>(expected.size()), &bytesWritten, nullptr);
  ...
  if (reader == nullptr) {
    std::cerr
        << "DefaultFileInterface failed to open a >512-character Unicode path."
        << std::endl;
    DeleteFileW(extendedFilePath.c_str());
    cleanup();
    return 1;
  ...
  if (bytesRead != expected.size() || actual != expected) {
    std::cerr << "DefaultFileInterface returned unexpected file contents."
              << std::endl;
    return 1;
  }

  std::cout << "Unicode >512-character path helpers and file I/O passed."
            << std::endl;
  return 0;
}
