...
inline std::wstring Utf8ToWide(const char *utf8) {
  ...
  if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8,
                          static_cast<int>(sourceLength), wide.data(),
                          wideLength) <= 0) {
    return {};
  }
...
inline std::wstring ToAbsolutePath(std::wstring path) {
  ...
  const auto written =
      GetFullPathNameW(path.c_str(), required, absolute.data(), nullptr);
...
inline std::u16string GetParentUtf16Path(const char16_t *path) {
  ...
  return separator == std::u16string::npos ? std::u16string{}
                                           : value.substr(0, separator);
}

inline std::u16string CombineUtf16Path(const char16_t *basePath,
                                       const char16_t *relativePath) {
  std::u16string combined;
  if (basePath != nullptr) {
    combined.assign(basePath);
  }

  if (!combined.empty() && combined.back() != u'/' &&
      combined.back() != u'\\') {
